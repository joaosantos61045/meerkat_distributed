...
use super::{
    eval_expr::{self, evaluate_expr},
    message,
    varworker::PendingWrite,
};
...
impl DefWorker {
    ...
    pub async fn tick(&mut self) {
        ...
        if (self.has_update_lock()) {
            ...
            let lock_granted_msg = Message::DefLockGranted {
                txn: update_lock.txn,
            };
    ...
    pub fn apply_code_update(&mut self) {
        ...
        for (name, op_data) in pcu.inputs_data.iter() {
            ...
            self.transtitive_deps
                .insert(name.to_string(), data.2.clone());
        }
    ...
    pub async fn handle_message(&mut self, msg: Message) {
        match msg {
            ...
            Message::DevWriteDefRequest { txn, write_expr } => {
                ...
                if !w_lock_granted {
                    ...
                    let added_inputs: HashSet<String> =
                        inputs.difference(&old_inputs).cloned().collect();
                    let deleted_inputs: HashSet<String> =
                        old_inputs.difference(&inputs).cloned().collect();
            ...
            Message::Subscribe {
                ...
                let respond_msg = Message::SubscriptionGranted {
                    ...
                    trans_dep_set: self.transtitive_deps.values().fold(
                        HashSet::new(),
                        |mut acc, set| {
            ...
            Message::SubscriptionGranted {
                ...
                pcu.inputs_data
                    .insert(name, Some((value, provides, trans_dep_set)));
    ...
    pub async fn run_defworker(mut self) {
        while let Some(msg) = self.def_rcvr.recv().await {
            ...
            let valid_batch = search_batch(&self.propa_changes_to_apply, &self.pred_txns);
...
async fn test_def_update_lock_precedence() {
    ...
    let update_msg = Message::DefLockRequest {
    ...
    let write_msg = Message::DefLockRequest {
        lock_kind: LockKind::Write,
        txn: Txn {
            id: write_txn.clone(),
            writes: vec![WriteToName {
                name: "f".to_string(),
                expr: Expr::IntConst { val: 3 },
            }],
        },
    };

    def_worker.handle_message(update_msg).await;
    def_worker.handle_message(write_msg).await;
